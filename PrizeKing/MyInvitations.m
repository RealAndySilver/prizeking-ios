...
-(void)viewDidLoad{
    ...
    scrollView.frame=CGRectMake(21, 91, anchoScroll, 204);
    ...
    CGRect invitationsLabelFrame=CGRectMake(21, 62, 200, 30);
    invitationsNumberLabel=[[UILabel alloc]initWithFrame:invitationsLabelFrame];
    invitationsNumberLabel.numberOfLines=4;
    invitationsNumberLabel.font=font3;
    invitationsNumberLabel.backgroundColor=[UIColor whiteColor];
    invitationsNumberLabel.textAlignment=UITextAlignmentCenter;
    invitationsNumberLabel.textColor=[UIColor blackColor];
    invitationsNumberLabel.layer.cornerRadius = 4.0;
    invitationsNumberLabel.layer.masksToBounds = YES;
    [invitationsNumberLabel.layer setBorderColor: [[UIColor blackColor] CGColor]];
    [invitationsNumberLabel.layer setBorderWidth: 2.0];
    [self.view addSubview:invitationsNumberLabel];
    ...
    [inviteButton addTarget:self action:@selector(accept) forControlEvents:UIControlEventTouchUpInside];
    inviteButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    [inviteButton setBackgroundImage:[self imageSetWithName:@"acceptInvitation.png"] forState:UIControlStateNormal];
...
-(void)buttonToggle{
    if (idArray.count==1) {
        ...
        friendName.alpha=0;
        infoLabel.text=@"Tap to choose a friend and accept his invitation.";
        infoLabel.alpha=1;
    }
}
-(void)startCall{
    ...
    invitationsNumberLabel.text=[NSString stringWithFormat:@"%i invitations",user.friendsList.count];
    NSLog(@"i = %i",i);
...
-(void)accept{
    peticion=2;
    NSString *parameters=[NSString stringWithFormat:@"%@/%@",[idArray objectAtIndex:0],user.ID];
    [server callServerWithMethod:@"ConfirmUserOnTeam" andParameter:parameters];
    hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    hud.labelText=@"Accepting Invitation";
}
-(void)getInvitations{
    peticion=1;
    [server callServerWithMethod:@"GetUserInvitations" andParameter:user.ID];
    hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    hud.labelText=@"Downloading Invitations";
}
...
-(void)receivedDataFromServer:(id)sender{
    ...
    if (peticion==1) {
        int i=0;
        invitationsNumberLabel.text=[NSString stringWithFormat:@"%i invitations",server.resDic.count];
        ...
        if (i==0) {
            ...
            label.text=@"You don't have invitations.";
